...
const skillMeta: Record<string, { name: string; nameEn: string; color: string; description: string }> = {
  rule: {
    ...
    color: '#a855f7',
    description: '深度解析市场规则文本，提取条件、时间约束和风险因素',
  },
  arbitrage: {
    name: '套利市场查询',
    nameEn: 'Arbitrage Market Query',
    color: '#00d4aa',
    description: '跨平台价格比对，发现实时套利机会',
  },
  news: {
    name: '新闻刮刀交易',
    nameEn: 'News Scraper Trading',
    color: '#4f88ff',
    description: '实时抓取全球媒体，自动生成交易信号',
  },
  copy: {
    name: '智能跟单',
    nameEn: 'Smart Copy Trading',
    color: '#ffa502',
    description: '追踪顶级预测者，自动复制高收益策略',
  },
  limit: {
    name: '挂单积分策略',
    nameEn: 'Limit Order Point Strategy',
    color: '#ff4757',
    description: '优化挂单策略以最大化积分奖励',
  },
  signal: {
    name: '个性信号推送',
    nameEn: 'Personalized Signal Push',
    color: '#06b6d4',
    description: '定制化高置信度信号，多渠道实时通知',
  },
}
...
export default function Home() {
  ...
  const SkillComponent = activeSkill ? skillComponents[activeSkill] : null

  return (
    <div className="min-h-screen bg-grid" style={{ background: '#080c14', backgroundImage: 'linear-gradient(rgba(79,136,255,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(79,136,255,0.03) 1px, transparent 1px)', backgroundSize: '40px 40px' }}>
      <Header onBack={activeSkill ? () => setActiveSkill(null) : undefined} activeSkill={meta?.name} />

      {!activeSkill ? (
        <main>
          <HeroStats />
          <SkillsGrid onSelectSkill={setActiveSkill} />
            ...
            <div className="flex items-center gap-4">
              <div>
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm px-2 py-0.5 rounded-lg" style={{ background: `${meta?.color}15`, border: `1px solid ${meta?.color}30`, color: meta?.color }}>
                    {meta?.nameEn}
                  </span>
                  <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-lg text-xs" style={{ background: 'rgba(0,212,170,0.08)', color: '#00d4aa' }}>
